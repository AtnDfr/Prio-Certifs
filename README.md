...
- **Objectif fixé** (décision COMEX, éditable et supprimable) : un paramètre totalement indépendant de l'objectif global ci-dessus — typiquement « combien de personnes on veut voir passer cette certification ce mois-ci ». Éditable en ligne dans la vue **Certifications** (colonne « Objectif fixé »), ou depuis la fiche détaillée — c'est le seul endroit où on le modifie, pour garder un point d'entrée unique par décision. La vue Providers reste focalisée sur la priorité (héritage/exception) ; elle affiche les certifications d'un fournisseur sans permettre d'y éditer l'objectif. Modifier l'objectif fixé ne change jamais l'objectif global affiché dans la colonne « Certifiés ». Il sert au calcul du reste à engager, du statut, et alimente la suggestion automatique de priorité (voir ci-dessus). La majorité des certifications restent volontairement sans objectif fixé.
